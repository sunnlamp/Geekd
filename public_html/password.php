...
if (isset($_POST['submitted'])) {
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT UserID FROM User WHERE (UserID=$e AND Password=('$p') )";
		//$q = "SELECT UserID FROM User WHERE (UserID=$e AND Password=SHA1('$p') )";
		...
		if ($num == 1) { // Match was made.
			...
			$q = "UPDATE User SET Password=('$np') WHERE UserID=$row[0]";
			//$q = "UPDATE User SET Password=SHA1('$np') WHERE UserID=$row[0]";		
			$r = @mysqli_query($dbc, $q);
